...
>>>>>>> 43c3f04efe0327ea627ebcedbd0e27a66688027f
...
<<<<<<< HEAD
grep $desiredHostname /etc/hosts || (printf "$desiredHostname Does not already exist.\n Changing hostname to $desiredHostname." &&
sudo sed -i "s/$currentHostname/$desiredHostname/" /etc/hosts)
# If that hostname is not the current hostname, change it using the hostnamectl command and
#     tell the user you changed the current hostname and they should reboot to make sure the new name takes full effect
#e.g. hostnamectl set-hostname $newname
test $currentHostname == $desiredHostname && echo "Your hostname is already named $desiredHostname." ||
hostnamectl set-hostname $desiredHostname && echo "Please restart your machine for the changes to take full effect."
=======
grep $desiredHostname /etc/hosts || (printf "$desiredHostname Does not already exist.\n Changing hostname to $desiredHostname." && sed -i "s/$currentHostname/$desiredHostname/" /etc/hosts)
# If that hostname is not the current hostname, change it using the hostnamectl command and
#     tell the user you changed the current hostname and they should reboot to make sure the new name takes full effect
#e.g. hostnamectl set-hostname $newname
